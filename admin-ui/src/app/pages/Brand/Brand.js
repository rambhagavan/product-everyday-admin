...
const Brand = () => {
  ...
  const fetchAllBrands = async (sortOrder = false, page = 1, limit = 10) => {
    setloading(true);
    const url = BACKEND_URL + '/brand'
    const params = {
      sortOrder: sortOrder,
      page: page,
      limit: limit
    ...
    if (data && data?.success === true) {
      ...
      settotalbrands(data?.data?.count)
      // dispatch(showSnackBar({ msg: "Get Brand Success", type: "success" }))
    } else {
      dispatch(showSnackBar({ msg: `Get Brand Fail ${data.exception_reason}`, type: "error" }))
    }
    resetStates()
    setloading(false);
  }
  ...
  const deleteBrand = async () => {
    setloading(true);
    const url = BACKEND_URL + '/brand/' + selectedBrand?._id
    const data = await remove(url)
    if (data && data?.success === true) {
      fetchAllBrands()
      dispatch(showSnackBar({ msg: "Delete Brand Success", type: "success" }))
    } else {
      dispatch(showSnackBar({ msg: `Delete Brand Fail ${data.exception_reason}`, type: "error" }))
    }
    setloading(false);
  }
  const handleChangePage = (page, pageSize) => {
    fetchAllBrands(false, page, 10)
    setCurrent(page);
  };
  ...
  const handleBulkAction = async () => {
    ...
    if (bulkaction === "delete") {
      const url = BACKEND_URL + '/brand/bulk'
  ...
  const onSearch = async (value, _e, info) => {
    ...
    setloading(true);
    const url = BACKEND_URL + '/brand/search'
    const params = {
      page: 1,
      limit: 10,
      q: value
        ...
        toggleNew === "new" ?
          <div>
            <div className="d-flex flex-row justify-content-between mb-4">
              <h5 className="modal-title fw-bolder">Create Brand</h5>
              <button type="button" class="btn-close" data-mdb-dismiss="modal" aria-label="Close" onClick={() => settoggleNew("list")}></button>
            </div>
            <CreateForm setloading={setloading} fetchAllBrands={fetchAllBrands} />
          </div> :
          toggleNew === "edit" ?
            <div>
              <div className="d-flex flex-row justify-content-between mb-4">
                <h5 class="modal-title fw-bolder" id="exampleModalLabel">Edit Brand</h5>
                <button type="button" class="btn-close" data-mdb-dismiss="modal" aria-label="Close" onClick={() => settoggleNew("list")}></button>
              </div>
              <EditForm brand={selectedBrand} setloading={setloading} fetchAllBrands={fetchAllBrands} />
            </div>
            :
            <div class="card mb-3">
              <div class="card-header">
                <div class="row align-items-center justify-content-between">
                  <div class="col-2 col-sm-auto d-flex align-items-center pr-0">
                    <h5 class="fs-0 mb-0 text-nowrap py-2 py-xl-0 me-2 fw-bolder">Brand</h5>
                  </div>
