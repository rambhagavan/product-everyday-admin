...
const CreateForm = ({ setloading, fetchAllProducts ,categoryName, categoryProduct }) => {
  ...
  const categories = useSelector(
    (state) => state.categoryListDataReducer.categoryList.data.categories
  );
  ...
  const handleCreateProduct = async () => {
    ...
    if (data && data?.success === true) {
      dispatch(
        showSnackBar({ msg: "Create Product Success", type: "success" })
      );
      fetchAllProducts();
    } else {
      dispatch(
        showSnackBar({
          msg: `Create Product Fail ${data.exception_reason}`,
          type: "error",
        })
      );
    }
