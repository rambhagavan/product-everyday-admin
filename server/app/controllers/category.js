...
const create = async (req, res) => {
  try {
    ...
    if (!description || !name) {
      return res
        .status(400)
        .send(errorResponse(400, null, "You must enter description & name."));
    }
...
const read = async (req, res) => {
  try {
    ...
    if (!categoryDoc) {
      return res
        .status(404)
        .send(errorResponse(404, null, "No Category found."));
    }

    res.status(200).send(successResponse(200, categoryDoc, null));
  } catch (err) {
    logger.fatal(err);
    return res
      .status(400)
...
const countCategories = async (req, res) => {
  try {
    ...
    const message= "total categories in our store"
    res.status(200).send(successResponse(200, categories, message));
  } catch (err) {
    logger.fatal(err);
    res
      .status(400)
    ...
    res
      ...
      .send(
        successResponse(
          200,
          responseList,
          "Category Bulk Update has been updated successfully!"
        )
      );
  } catch (err) {
    logger.fatal(err);
...
const listActive = async (req, res) => {
  try {
    ...
    res.status(200).send(successResponse(200, categories, null));
  } catch (err) {
    logger.fatal(err);
    res
      .status(400)
